...
def load_challenge_data2(data_folder, patient_id):
    ...
    recording_patient=np.zeros((18,1))
    for recording_id in recording_ids:
        if recording_id != 'nan':
            ...
            try:
                ...
                #print(recording_patient.shape)
                recording_patient = np.concatenate((recording_patient, recording_data), axis=1)
            except:
            ...
            channels = None
    recordings.append(recording_patient)    
...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    #model  = EEGNet(nb_classes = 5, Chans = 18, Samples = 30000)
    #"""
    timesteps = 30000
    channels_num = 18

    model = keras.Sequential()
    model.add(LSTM(256,input_shape=(channels_num,timesteps), return_sequences=True))
    model.add(LSTM(128, return_sequences=True))
    model.add(LSTM(64, return_sequences=False))
    model.add(Dense(5, activation='softmax'))#"""

    model.summary()
    model.compile(loss = 'categorical_crossentropy', optimizer = 'adam',metrics=[keras.metrics.TruePositives()])
    ...
    fittedModel = model.fit(total_data, to_categorical(cpcs,num_classes=5), batch_size=16, epochs=20,callbacks=[tensorboard_callback, callback]) 
    model.save(os.path.join(model_folder, 'LSTM_total.sav'))
...
def load_challenge_models(model_folder, verbose):
    new_model = keras.models.load_model(os.path.join(model_folder, 'LSTM_total.sav'))
    return new_model
